...
def index():
    ...
    if cities:
        for city in cities:
            link = "http://api.openweathermap.org/data/2.5/weather?q=%s&APPID=be04d17b9eb743ac0b4cce14a33685c0&units=metric" % city.name
            r = requests.get(link)
            weather = r.json()
            print(weather)
    ...
    if city:
        if exists:
            flash("The city has already been added to the list!")
                ...
                "http://api.openweathermap.org/data/2.5/weather?q=%s&APPID=be04d17b9eb743ac0b4cce14a33685c0&units=metric" % city).status_code != 200:
